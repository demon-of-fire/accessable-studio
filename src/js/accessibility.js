...
const Accessibility = (() => {
  ...
  /** Trap focus within a modal dialog (dynamically queries focusable elements each time) */
  function trapFocus(element) {
    const focusableSelectors = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';

    function getFocusable() {
      // Only return elements that are visible (not hidden)
      const all = element.querySelectorAll(focusableSelectors);
      return Array.from(all).filter(el => el.offsetParent !== null && !el.disabled);
    }
